...
状态：阶段 2 已完成；阶段 3 的金、银、玫瑰金与局部短绒毛绒样本已完成
基线日期：2026-07-30
...
3. 在普通柔雾颜色之外增加金、银、玫瑰金、局部短绒毛绒和珠光闪粉表面。
...
- 24 只正式模型 ID 已收敛到 `formalColorAnimalModelIds`，目录、抽取池与能力
  注册表共享这一清单；
- `/appearance-lab/` 已提供柔雾与局部短绒各 24 × 9、金/银/玫瑰金各
  24 个样本、系列/模型/配色/表面筛选及单个实时 3D 检查器；
...
- 金、银、玫瑰金与局部短绒样本已完成；珠光闪粉与奖励规则仍待后续阶段。
...
type ToySurfaceStyle =
  ...
  | { kind: "plush"; fiberScale: number; fuzzStrength: number }
...
- 金属与毛绒只改变模型已批准的换色区域，不覆盖身体和五官。
- 金属档读取独立的 metalness、roughness、envMapIntensity 和 clearcoat。
- 毛绒档复用同一局部遮罩，以高粗糙度、低环境反射、稳定物体空间短绒明暗和
  边缘绒光表达材质。
...
第一轮先完成三种金属与局部短绒样本；下一轮再做一个低强度珠光闪粉样本。
所有新表面都不直接进入抽取经济，先在
...
- shader program cache key 与缩略图 appearance signature 均包含表面身份，避免
  柔雾、三种金属与毛绒之间互相串用缓存。
- `plush` 复用同一局部遮罩，按 detail / compact / tile / thumbnail 分档配置
  短绒尺度、纤维明暗与边缘绒光；配色仍来自九组常规色板。
- Appearance Lab 已重组为左侧“柔雾树脂 / 金属 / 毛绒”表面材质与右侧
  配色控制：金属显示金/银/玫瑰金，柔雾和毛绒显示九组常规配色，
  总计 504 种组合。
- 本地 Lab 已完成柔雾、局部毛绒与三种金属各 24/24 的真实浏览器检查，
  并完成 359px 手机视口无页面横向溢出检查。
