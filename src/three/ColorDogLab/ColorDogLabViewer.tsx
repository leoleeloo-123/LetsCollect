...
type Props = { variant: DogColorVariant };
...
function colorizeCoat(
  ...
  material.onBeforeCompile = (shader) => {
    shader.uniforms.dogProtectMap = { value: protectMap };
    shader.uniforms.dogCoatColor = { value: coatColor };
    shader.fragmentShader = shader.fragmentShader
      .replace("#include <common>", `#include <common>
uniform sampler2D dogProtectMap;
uniform vec3 dogCoatColor;`)
      .replace("#include <map_fragment>", `#ifdef USE_MAP
  vec4 sampledDiffuseColor = texture2D(map, vMapUv);
  vec3 originalDiffuseColor = sampledDiffuseColor.rgb;
  vec3 detailMask = texture2D(dogProtectMap, vMapUv).rgb;
  ...
  vec3 colorizedCoat = dogCoatColor * originalContrast;
  float coatAmount = 1.0 - protectedDetail;
  sampledDiffuseColor.rgb = mix(sampledDiffuseColor.rgb, colorizedCoat, coatAmount);
  ...
  material.customProgramCacheKey = () => "color-dog-protected-coat-v14";
  material.needsUpdate = true;
}

export function ColorDogLabViewer({ variant }: Props) {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const controls: CoatControls = {
        color: new THREE.Color(variant.swatch).multiplyScalar(0.86),
      ...
      model.traverse((child) => {
        ...
        const clones = originals.map((item) => {
          ...
          colorizeCoat(clone, controls.color, protectMap);
    ...
    setup().catch((error) => {
      console.error("[ColorDogLabViewer] color dog load failed", error);
  ...
  return (
    <div className="color-dog-viewer color-dog-viewer--single" role="group" aria-label={`${variant.name} 3D 小狗`}>
      <div ref={hostRef} className="color-dog-viewer__host" />
      <div className="color-dog-single__palette" aria-label="当前随机毛色">
        <span style={{ background: variant.swatch }} /><strong>{variant.name}</strong>
      </div>
      ...
      {status !== "ready" ? (
        <div className={`color-dog-viewer__status color-dog-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-dog-viewer__spinner" /><strong>正在加载 Color Dog</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
