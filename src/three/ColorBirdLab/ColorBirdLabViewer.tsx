...
} from "../material/createColorBirdCrownMaterials";
import {
  loadRoomEnvironment,
  loadToyModel,
...
type Props = {
  variant: BirdCrownVariant;
  showZones: boolean;
};

type Status = "loading" | "ready" | "error";

type BirdCrownControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-bird/model-mobile-v002.glb";
const MASK_URL =
  "/models/toys/color-bird/crown-mask-mobile-v001.webp?v=11";
const TRIANGLE_MASK_URL =
  "/models/toys/color-bird/crown-triangle-mask-mobile-v001.bin?v=11";

async function loadCrownTriangleMask() {
  const response = await fetch(TRIANGLE_MASK_URL);
  if (!response.ok) {
    throw new Error(`皇冠三角面遮罩加载失败：${response.status}`);
  }
...
export function ColorBirdLabViewer({ variant, showZones }: Props) {
  ...
  const controlsRef = useRef<BirdCrownControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const [crownMask, crownTriangleMask] = await Promise.all([
        new THREE.TextureLoader().loadAsync(MASK_URL),
        loadCrownTriangleMask()
      ]);
      if (cancelled || !hostRef.current) {
        crownMask.dispose();
        return;
      }
      prepareColorBirdCrownMaskTexture(THREE, crownMask);
      ...
      renderer.setPixelRatio(
        Math.min(window.devicePixelRatio, compact ? 1.35 : 1.6)
      );
      ...
      scene.add(new THREE.HemisphereLight(0xffffff, 0x776a64, 1.95));
      const key = new THREE.DirectionalLight(0xfffbf7, 1.7);
      key.position.set(-4.5, 6.2, 7);
      scene.add(key);
      const fill = new THREE.DirectionalLight(0xffe3ee, 0.38);
      ...
      const inspectionRotation = Number.isFinite(rotationValue)
        ...
        : null;
      const controls: BirdCrownControls = {
      ...
      const materials = cloneColorBirdCrownMaterials(
        THREE,
        model,
        controls.color,
        crownMask,
      ...
      stage.rotation.y = inspectionRotation ?? -0.08;
      stage.add(model);
      scene.add(stage);
      ...
      let targetRotationY = inspectionRotation ?? -0.08;
      ...
      function render(time: number) {
        ...
        if (
          document.hidden ||
          time - lastFrame < 1000 / (dragging ? 60 : 30)
        ) {
          return;
        }
        ...
        const moving =
          Math.abs(rotationX - targetRotationX) > 0.0005 ||
    ...
    setup().catch((error) => {
      console.error("[ColorBirdLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-bird-viewer"
      role="group"
      aria-label={`${variant.name} Color Bird 3D`}
      data-status={status}
    >
      ...
      <div
        ...
        <strong>{showZones ? "皇冠区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
