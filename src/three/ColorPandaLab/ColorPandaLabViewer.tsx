...
type Props = { variant: PandaHatVariant; showZones: boolean };
...
export function ColorPandaLabViewer({ variant, showZones }: Props) {
  ...
  useEffect(() => {
    ...
    setup().catch((error) => {
      console.error("[ColorPandaLabViewer] color panda load failed", error);
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-panda-viewer" role="group" aria-label={`${variant.name} 3D 熊猫`}>
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Panda</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
