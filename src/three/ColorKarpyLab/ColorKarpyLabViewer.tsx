...
export type KarpyHatVariant = {
  id: string;
  name: string;
  swatch: string;
};

type Props = { variant: KarpyHatVariant; showZones: boolean };
...
const MODEL_URL = "/models/toys/color-karpy/model-mobile-v001.glb";
const PROTECT_MASK_URL = "/models/toys/color-karpy/hat-mask-mobile-v001.webp?v=2";
...
export function ColorKarpyLabViewer({ variant, showZones }: Props) {
  ...
  useEffect(() => {
    ...
    setup().catch((error) => {
      console.error("[ColorKarpyLabViewer] color karpy load failed", error);
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-karpy-viewer" role="group" aria-label={`${variant.name} 3D Karpy`}>
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Karpy</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
