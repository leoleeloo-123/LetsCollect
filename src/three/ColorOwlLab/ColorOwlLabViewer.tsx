...
const MODEL_URL = "/models/toys/color-owl/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-owl/hat-book-mask-mobile-v001.webp?v=2";

export function ColorOwlLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<OwlControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const academicMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        academicMask.dispose();
        return;
      }
      prepareColorOwlMaskTexture(THREE, academicMask);
      ...
      const materials = cloneColorOwlMaterials(
        THREE,
        model,
        controls.color,
        academicMask,
    ...
    setup().catch((error) => {
      console.error("[ColorOwlLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-owl-viewer"
      role="group"
      aria-label={`${variant.name} 3D Owl`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前博士帽与书皮颜色">
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "博士帽与书皮区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
