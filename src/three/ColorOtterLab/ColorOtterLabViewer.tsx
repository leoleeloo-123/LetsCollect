...
export type OtterLollipopVariant = {
  id: string;
  name: string;
  swatch: string;
};

type Props = { variant: OtterLollipopVariant; showZones: boolean };
type Status = "loading" | "ready" | "error";
type LollipopControls = {
  materials: import("three").MeshStandardMaterial[];
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-otter/model-mobile-v008.glb?v=2";
const LOLLIPOP_MATERIAL_NAME = "Lollipop_Color";
const DEBUG_COLOR = "#2446c7";

function setLollipopColor(controls: LollipopControls, swatch: string, showZones: boolean) {
  controls.materials.forEach((material) => {
    material.color.set(showZones ? DEBUG_COLOR : swatch);
    material.needsUpdate = true;
  });
  controls.invalidate();
}

export function ColorOtterLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<LollipopControls | null>(null);
  ...
  useEffect(() => {
    ...
    if (controls) setLollipopColor(controls, variant.swatch, showZones);
  }, [showZones, variant]);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const controls: LollipopControls = {
        materials: [],
      ...
      model.traverse((child) => {
        ...
        const clones = originals.map((item) => {
          ...
          if (clone instanceof THREE.MeshStandardMaterial) {
            clone.metalness = 0;
            if (clone.name === LOLLIPOP_MATERIAL_NAME) {
              clone.map = null;
              clone.metalnessMap = null;
              clone.roughnessMap = null;
              clone.roughness = 0.58;
              clone.envMapIntensity = 0.5;
              controls.materials.push(clone);
            } else {
              clone.roughness = 1;
      ...
      if (controls.materials.length === 0) {
        throw new Error(`Color Otter material ${LOLLIPOP_MATERIAL_NAME} was not found`);
      }
      setLollipopColor(controls, variantRef.current.swatch, showZonesRef.current);
    ...
    setup().catch((error) => {
      console.error("[ColorOtterLabViewer] color otter load failed", error);
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-otter-viewer" role="group" aria-label={`${variant.name} 3D 水獭`}>
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前棒棒糖颜色">
        <span style={{ background: variant.swatch }} /><strong>{showZones ? "着色区域检查" : variant.name}</strong>
      </div>
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Otter</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
