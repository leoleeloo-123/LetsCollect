...
export type SealStarfishVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type ColorSealControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-seal/model-mobile-v001.glb?v=1";
const MASK_URL = "/models/toys/color-seal/starfish-mask-mobile-v001.webp?v=9";
const OBJECT_MASK_URL = "/models/toys/color-seal/starfish-object-mask-mobile-v001.webp?v=8";

export function ColorSealLabViewer({
  variant,
  showZones,
  compact = false,
  interactive = true,
  ...
  const controlsRef = useRef<ColorSealControls | null>(null);
  ...
  useEffect(() => {
    ...
    if (!controls) return;
    controls.color.set(variant.swatch).multiplyScalar(0.9);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const [starfishMask, starfishObjectMask] = await Promise.all([
        new THREE.TextureLoader().loadAsync(MASK_URL),
        new THREE.TextureLoader().loadAsync(OBJECT_MASK_URL)
      ]);
      if (cancelled || !hostRef.current) {
        starfishMask.dispose();
        starfishObjectMask.dispose();
        return;
      }
      prepareColorSealMaskTexture(THREE, starfishMask);
      prepareColorSealObjectMaskTexture(THREE, starfishObjectMask);
      ...
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, compact ? 1.3 : 1.6));
      ...
      scene.add(new THREE.HemisphereLight(0xffffff, 0x776a63, 1.9));
      ...
      scene.add(fill);
      const rim = new THREE.DirectionalLight(0xdceaff, 0.34);
      rim.position.set(4, 3.5, -5);
      scene.add(rim);

      let needsRender = true;
      const controls: ColorSealControls = {
        color: new THREE.Color(variantRef.current.swatch).multiplyScalar(0.9),
      ...
      const materials = cloneColorSealMaterials(
        THREE,
        model,
        controls.color,
        starfishMask,
      ...
      const scale = 3.45 / Math.max(initialSize.x, initialSize.y, initialSize.z, 0.001);
      ...
      stage.rotation.set(-0.02, rotationController?.getRotation() ?? -0.28, 0);
      ...
      let targetRotationY = rotationController?.getRotation() ?? -0.28;
      ...
      function handleWheel(event: WheelEvent) {
        ...
        targetZoom = THREE.MathUtils.clamp(targetZoom + event.deltaY * 0.004, 5.2, 8.4);
        idleUntil = performance.now() + 5200;
        needsRender = true;
      }
    ...
    setup().catch((error) => {
      console.error("[ColorSealLabViewer] model load failed", error);
  ...
  return (
    <div
      className={`color-animal-viewer color-animal-viewer--single color-seal-viewer${compact ? " color-seal-viewer--compact" : ""}`}
      role="group"
      aria-label={`${variant.name} Color Seal 3D`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      {!compact ? (
        <div className="color-animal-single__palette" aria-label="当前海星颜色">
          <span style={{ background: variant.swatch }} />
          <strong>{showZones ? "海星区域检查" : variant.name}</strong>
        </div>
      ) : null}
