...
RUNTIME_DIR = ROOT / "public" / "models" / "toys" / "color-karpy"
...
def build_mask() -> tuple[Image.Image, float]:
    ...
    for y in range(MASK_SIZE):
        for x in range(MASK_SIZE):
            ...
            strength = min(
                ramp(red, 52.0, 80.0),
                ramp(red - green, 38.0, 80.0),
                ramp(red - blue, 18.0, 70.0),
                1.0 - ramp(green, 38.0, 60.0),
            )
            value = round(255 * strength)
            target[x, y] = value

    hat = hat.filter(ImageFilter.MaxFilter(7))
    hat = hat.filter(ImageFilter.MinFilter(7))
    hat = hat.point(
        lambda value: 0 if value < 40 else round((value - 40) * 255 / 215)
    )
    hat = hat.filter(ImageFilter.GaussianBlur(0.65))
    empty = Image.new("L", base.size, 0)
    coverage = sum(
        value * count for value, count in enumerate(hat.histogram())
    ) / (255 * MASK_SIZE * MASK_SIZE)
    return Image.merge("RGB", (hat, empty, empty)), coverage
