...
OUTPUT = RUNTIME_DIR / "bath-mask-mobile-v003.webp"
MASK_SIZE = 1024
...
def build_mask() -> tuple[Image.Image, float]:
    ...
    foam_protection = Image.new("L", base.size, 255)
    source = base.load()
    target = mask.load()
    protected = foam_protection.load()

    for y in range(MASK_SIZE):
        for x in range(MASK_SIZE):
            red, green, blue = source[x, y]
            bath_strength = min(
                ramp(red - green, 43.0, 68.0),
                ramp(blue - green, 18.0, 52.0),
                ramp(red + blue - green * 2.0, 100.0, 120.0),
            )
            target[x, y] = round(255 * bath_strength)
            protected[x, y] = round(255 * (1.0 - ramp(green, 100.0, 35.0)))

    mask = mask.filter(ImageFilter.MaxFilter(3))
    mask = mask.filter(ImageFilter.MinFilter(3))
    mask = mask.filter(ImageFilter.GaussianBlur(0.35))
    mask = ImageChops.multiply(mask, foam_protection)
