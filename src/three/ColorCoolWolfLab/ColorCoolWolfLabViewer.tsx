...
} from "../material/createColorCoolWolfMaterials";
import {
  loadRoomEnvironment,
  loadToyModel,
...
export type CoolWolfStudVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type Props = {
  variant: CoolWolfStudVariant;
  showZones: boolean;
};

type Status = "loading" | "ready" | "error";

type CoolWolfControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-cool-wolf/model-mobile-v001.glb";
const MASK_URL =
  "/models/toys/color-cool-wolf/ear-stud-mask-mobile-v001.webp?v=3";

export function ColorCoolWolfLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<CoolWolfControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const studMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        studMask.dispose();
        return;
      }
      prepareColorCoolWolfMaskTexture(THREE, studMask);
      ...
      const inspectionRotation = Number.isFinite(rotationValue)
        ...
        : null;
      const controls: CoolWolfControls = {
      ...
      const materials = cloneColorCoolWolfMaterials(
        THREE,
        model,
        controls.color,
        studMask,
    ...
    setup().catch((error) => {
      console.error("[ColorCoolWolfLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-cool-wolf-viewer"
      role="group"
      aria-label={`${variant.name} Color Cool Wolf 3D`}
      data-status={status}
    >
      ...
      <div
        ...
        <strong>{showZones ? "三枚耳钉区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Cool Wolf</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
