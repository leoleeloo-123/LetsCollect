...
export function ColorPandaLabViewer({ variant, showZones }: Props) {
  ...
  return (
    <div className="color-dog-viewer color-dog-viewer--single color-panda-viewer" role="group" aria-label={`${variant.name} 3D 熊猫`}>
      <div ref={hostRef} className="color-dog-viewer__host" />
      <div className="color-dog-single__palette" aria-label="当前帽子颜色">
      ...
      {status !== "ready" ? (
        <div className={`color-dog-viewer__status color-dog-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-dog-viewer__spinner" /><strong>正在加载 Color Panda</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
