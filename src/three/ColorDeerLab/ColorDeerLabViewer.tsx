...
const MODEL_URL = "/models/toys/color-deer/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-deer/accessory-mask-mobile-v001.webp?v=22";

export function ColorDeerLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<DeerControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorDeerMaskTexture(THREE, accessoryMask);
      ...
      const materials = cloneColorDeerMaterials(
        THREE,
        model,
        controls.color,
        accessoryMask,
    ...
    setup().catch((error) => {
      console.error("[ColorDeerLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-deer-viewer"
      role="group"
      aria-label={`${variant.name} 3D Deer`}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前蝴蝶结与包包颜色">
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "配件区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
