...
export type DinoScarfVariant = {
  id: string;
  name: string;
  swatch: string;
};

type Props = { variant: DinoScarfVariant; showZones: boolean };
...
const MODEL_URL = "/models/toys/color-dino/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-dino/scarf-mask-mobile-v001.webp?v=2";

function colorizeScarf(
  material: import("three").Material,
  scarfColor: import("three").Color,
  scarfMask: import("three").Texture,
  debugMode: { value: number }
) {
  material.onBeforeCompile = (shader) => {
    shader.uniforms.dinoScarfMask = { value: scarfMask };
    shader.uniforms.dinoScarfColor = { value: scarfColor };
    shader.uniforms.dinoScarfDebugMode = debugMode;
    shader.vertexShader = shader.vertexShader
      .replace("#include <common>", `#include <common>
varying vec3 vDinoObjectPosition;`)
      .replace("#include <begin_vertex>", `#include <begin_vertex>
vDinoObjectPosition = position;`);
    shader.fragmentShader = shader.fragmentShader
      .replace("#include <common>", `#include <common>
uniform sampler2D dinoScarfMask;
uniform vec3 dinoScarfColor;
uniform float dinoScarfDebugMode;
varying vec3 vDinoObjectPosition;`)
      .replace("#include <map_fragment>", `#ifdef USE_MAP
  vec4 sampledDiffuseColor = texture2D(map, vMapUv);
  vec3 originalDiffuseColor = sampledDiffuseColor.rgb;
  float scarfCandidate = smoothstep(0.06, 0.38, texture2D(dinoScarfMask, vMapUv).r);
  float scarfX = smoothstep(-0.320, -0.292, vDinoObjectPosition.x)
    * (1.0 - smoothstep(0.360, 0.390, vDinoObjectPosition.x));
  float scarfY = smoothstep(-0.265, -0.235, vDinoObjectPosition.y)
    * (1.0 - smoothstep(0.123, 0.155, vDinoObjectPosition.y));
  float scarfZ = smoothstep(-0.370, -0.344, vDinoObjectPosition.z)
    * (1.0 - smoothstep(0.303, 0.333, vDinoObjectPosition.z));
  float scarfDetail = scarfCandidate * scarfX * scarfY * scarfZ;

  float baseLuma = dot(originalDiffuseColor, vec3(0.2126, 0.7152, 0.0722));
  float scarfShading = mix(0.48, 1.08, smoothstep(0.04, 0.92, baseLuma));
  vec3 colorizedScarf = dinoScarfColor * scarfShading;
  vec3 resultColor = mix(originalDiffuseColor, colorizedScarf, scarfDetail);
  ...
  sampledDiffuseColor.rgb = mix(resultColor, zoneColor, dinoScarfDebugMode);
  diffuseColor *= sampledDiffuseColor;
#endif`);
  };
  material.customProgramCacheKey = () => "color-dino-scarf-v1";
  material.needsUpdate = true;
}
export function ColorDinoLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<ScarfControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const scarfMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      scarfMask.flipY = false;
      scarfMask.colorSpace = THREE.NoColorSpace;
      scarfMask.wrapS = THREE.RepeatWrapping;
      scarfMask.wrapT = THREE.RepeatWrapping;
      scarfMask.generateMipmaps = false;
      scarfMask.minFilter = THREE.LinearFilter;
      ...
      model.traverse((child) => {
        ...
        const clones = originals.map((item) => {
          ...
          colorizeScarf(clone, controls.color, scarfMask, controls.debugMode);
    ...
    setup().catch((error) => {
      console.error("[ColorDinoLabViewer] color dino load failed", error);
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-dino-viewer" role="group" aria-label={`${variant.name} 3D Dino`}>
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前围脖颜色">
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Dino</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
