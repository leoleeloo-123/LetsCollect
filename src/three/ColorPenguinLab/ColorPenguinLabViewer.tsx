...
} from "../material/createColorPenguinMaterials";
import {
  loadRoomEnvironment,
  loadToyModel,
...
export type PenguinAccessoryVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type Props = {
  variant: PenguinAccessoryVariant;
  showZones: boolean;
};

type Status = "loading" | "ready" | "error";

type PenguinControls = {
...
export function ColorPenguinLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<PenguinControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorPenguinMaskTexture(THREE, accessoryMask);
      ...
      const inspectionRotation = Number.isFinite(rotationValue)
        ...
        : null;
      const controls: PenguinControls = {
      ...
      const materials = cloneColorPenguinMaterials(
        THREE,
        model,
        controls.color,
        accessoryMask,
    ...
    setup().catch((error) => {
      console.error("[ColorPenguinLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-penguin-viewer"
      role="group"
      aria-label={`${variant.name} Color Penguin 3D`}
      data-status={status}
    >
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Penguin</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
