...
export type BearSingerAfroVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type BearSingerControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-bear-singer/model-mobile-v006.glb?v=6";
const MASK_URL = "/models/toys/color-bear-singer/afro-mask-mobile-v001.webp?v=1";

export function ColorBearSingerLabViewer({
  variant,
  showZones,
  compact = false,
  interactive = true,
  ...
  const controlsRef = useRef<BearSingerControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const afroMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        afroMask.dispose();
        return;
      }
      prepareColorBearSingerMaskTexture(THREE, afroMask);
      ...
      const controls: BearSingerControls = {
      ...
      const materials = cloneColorBearSingerMaterials(
        THREE,
        model,
        controls.color,
        afroMask,
    ...
    setup().catch((error) => {
      console.error("[ColorBearSingerLabViewer] model load failed", error);
  ...
  return (
    <div
      className={`color-animal-viewer color-animal-viewer--single color-bear-singer-viewer${compact ? " color-bear-singer-viewer--compact" : ""}`}
      role="group"
      aria-label={`${variant.name} 3D 歌手小熊`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      {!compact ? (
        <div className="color-animal-single__palette" aria-label="当前爆炸头颜色">
          <span style={{ background: variant.swatch }} />
          <strong>{showZones ? "顶部爆炸头区域检查" : variant.name}</strong>
        </div>
      ) : null}
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Bear Singer</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
