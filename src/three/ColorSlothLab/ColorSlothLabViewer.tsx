...
const MODEL_URL = "/models/toys/color-sloth/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-sloth/hat-mask-mobile-v001.webp?v=4";

export function ColorSlothLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<SlothControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorSlothMaskTexture(THREE, hatMask);
      ...
      const materials = cloneColorSlothMaterials(
        THREE,
        model,
        controls.color,
        hatMask,
    ...
    setup().catch((error) => {
      console.error("[ColorSlothLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-sloth-viewer"
      role="group"
      aria-label={`${variant.name} 3D Sloth`}
      data-status={status}
    >
      ...
      <div className="color-animal-single__palette" aria-label="当前帽子颜色">
        ...
        <strong>{showZones ? "帽子区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Sloth</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
