...
CONTACT_PATCH_MIN = np.array([0.270, 0.100, 0.160], dtype=np.float32)
...
CONTACT_PATCH_ELLIPSOID_RADII = np.array([0.027, 0.145, 0.120], dtype=np.float32)
CONTACT_PATCH_SECONDARY_CENTER = np.array([0.310, 0.240, 0.315], dtype=np.float32)
CONTACT_PATCH_SECONDARY_RADII = np.array([0.035, 0.065, 0.060], dtype=np.float32)
CONTACT_PATCH_ELLIPSOID_THRESHOLD = 1.0
CONTACT_PATCH_INSET_FACTOR = 0.12
...
def split_polygon_ellipsoid(
    ...
    def position_field(position: np.ndarray) -> float:
        primary = (position - CONTACT_PATCH_ELLIPSOID_CENTER) / CONTACT_PATCH_ELLIPSOID_RADII
        secondary = (position - CONTACT_PATCH_SECONDARY_CENTER) / CONTACT_PATCH_SECONDARY_RADII
        return min(
            float(np.dot(primary, primary) - CONTACT_PATCH_ELLIPSOID_THRESHOLD),
            float(np.dot(secondary, secondary) - CONTACT_PATCH_ELLIPSOID_THRESHOLD),
        )

    def field(index: int) -> float:
        return position_field(position_rows[index])
    ...
    def append_intersection(left: int, right: int) -> int:
        ...
        for _ in range(28):
            ...
            middle_inside = position_field(position) <= 0.0
...
def split_contact_patch(
    ...
    for triangle in triangles[target_mask]:
        ...
        if inside_polygons:
            ...
            for polygon in inside_polygons:
                inset_polygon: list[int] = []
                for index in polygon:
                    position = position_rows[index]
                    inset_position = position + (
                        CANDY_SURFACE_CENTER - position
                    ) * CONTACT_PATCH_INSET_FACTOR
                    position_rows.append(inset_position.astype(np.float32))
                    normal_rows.append(normal_rows[index].copy())
                    uv_rows.append(uv_rows[index].copy())
                    inset_polygon.append(len(position_rows) - 1)
                clipped = triangulate_polygon(inset_polygon)
...
def find_lower_candy_cap(
    ...
    component_mask = triangle_roots == target_root
    connector_tip_relative = (
        centroids - np.array([0.315, 0.075, 0.190], dtype=np.float32)
    ) / np.array([0.075, 0.070, 0.110], dtype=np.float32)
    connector_tip_mask = (
        component_mask
        & (np.square(connector_tip_relative).sum(axis=1) < 1.0)
        & (centroids[:, 1] > 0.0)
    )
    return (
        (
