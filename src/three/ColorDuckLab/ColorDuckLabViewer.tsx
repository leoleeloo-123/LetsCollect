...
const MODEL_URL = "/models/toys/color-duck/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-duck/bath-mask-mobile-v003.webp?v=1";
const FOAM_CLEANUP_MASK_URL =
  "/models/toys/color-duck/foam-cleanup-mask-mobile-v001.webp?v=1";

export function ColorDuckLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<DuckControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const [bathMask, foamCleanupMask] = await Promise.all([
        textureLoader.loadAsync(MASK_URL),
        textureLoader.loadAsync(FOAM_CLEANUP_MASK_URL)
      ]);
      if (cancelled || !hostRef.current) {
        bathMask.dispose();
        foamCleanupMask.dispose();
        return;
      }
      prepareColorDuckMaskTexture(THREE, bathMask);
      prepareColorDuckMaskTexture(THREE, foamCleanupMask);
      ...
      const materials = cloneColorDuckMaterials(
        THREE,
        model,
        controls.color,
        bathMask,
    ...
    setup().catch((error) => {
      console.error("[ColorDuckLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-duck-viewer"
      role="group"
      aria-label={`${variant.name} 3D Duck`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前浴缸与浴帽颜色">
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "浴缸与浴帽区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
