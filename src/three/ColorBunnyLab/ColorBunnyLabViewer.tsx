...
export function ColorBunnyLabViewer({ variant, showZones }: Props) {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorBunnyProtectTexture(THREE, protectMap);
      ...
      const materials = cloneColorBunnyMaterials(
        THREE,
        model,
        controls.color,
        protectMap,
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-bunny-viewer" role="group" aria-label={`${variant.name} 3D 小兔`}>
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前包包颜色">
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Bunny</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
