...
} from "../material/createColorBlackCatMaterials";
import {
  loadRoomEnvironment,
  loadToyModel,
...
export type BlackCatLogoVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type Props = {
  variant: BlackCatLogoVariant;
  showZones: boolean;
};

type Status = "loading" | "ready" | "error";

type BlackCatControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-black-cat/model-mobile-v001.glb";
const MASK_URL =
  "/models/toys/color-black-cat/fish-logo-mask-mobile-v001.webp?v=5";

export function ColorBlackCatLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<BlackCatControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const logoMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        logoMask.dispose();
        return;
      }
      prepareColorBlackCatMaskTexture(THREE, logoMask);
      ...
      const inspectionRotation = Number.isFinite(rotationValue)
        ...
        : null;
      const controls: BlackCatControls = {
      ...
      const materials = cloneColorBlackCatMaterials(
        THREE,
        model,
        controls.color,
        logoMask,
      ...
      stage.rotation.y = inspectionRotation ?? -0.22;
      stage.add(model);
      scene.add(stage);
      ...
      let targetRotationY = inspectionRotation ?? -0.22;
    ...
    setup().catch((error) => {
      console.error("[ColorBlackCatLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-black-cat-viewer"
      role="group"
      aria-label={`${variant.name} Color Black Cat 3D`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div
        className="color-animal-single__palette"
        aria-label="当前鱼 Logo 颜色">
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "鱼 Logo 区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Black Cat</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
