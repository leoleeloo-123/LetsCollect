...
const MODEL_URL = "/models/toys/color-sheep/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-sheep/accessory-mask-mobile-v001.webp?v=4";

export function ColorSheepLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<SheepControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorSheepMaskTexture(THREE, accessoryMask);
      ...
      const materials = cloneColorSheepMaterials(
        THREE,
        model,
        controls.color,
        accessoryMask,
    ...
    setup().catch((error) => {
      console.error("[ColorSheepLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-sheep-viewer"
      role="group"
      aria-label={`${variant.name} 3D Sheep`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前披风与蝴蝶结颜色">
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "披风与蝴蝶结区域" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Sheep</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
