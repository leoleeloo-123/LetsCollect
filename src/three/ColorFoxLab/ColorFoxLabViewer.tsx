...
export type FoxHatVariant = {
  id: string;
  name: string;
  swatch: string;
};

type Props = { variant: FoxHatVariant; showZones: boolean };
...
const MODEL_URL = "/models/toys/color-fox/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-fox/hat-feather-mask-mobile-v001.webp?v=4";

function colorizeHatFeather(
  material: import("three").Material,
  hatColor: import("three").Color,
  hatMask: import("three").Texture,
  debugMode: { value: number }
) {
  material.onBeforeCompile = (shader) => {
    shader.uniforms.foxHatMask = { value: hatMask };
    shader.uniforms.foxHatColor = { value: hatColor };
    shader.uniforms.foxHatDebugMode = debugMode;
    shader.vertexShader = shader.vertexShader
      .replace("#include <common>", `#include <common>
varying vec3 vFoxObjectPosition;`)
      .replace("#include <begin_vertex>", `#include <begin_vertex>
vFoxObjectPosition = position;`);
    shader.fragmentShader = shader.fragmentShader
      .replace("#include <common>", `#include <common>
uniform sampler2D foxHatMask;
uniform vec3 foxHatColor;
uniform float foxHatDebugMode;
varying vec3 vFoxObjectPosition;`)
      .replace("#include <map_fragment>", `#ifdef USE_MAP
  vec4 sampledDiffuseColor = texture2D(map, vMapUv);
  vec3 originalDiffuseColor = sampledDiffuseColor.rgb;
  vec2 foxTargetMask = texture2D(foxHatMask, vMapUv).rg;
  float hatCandidate = smoothstep(0.06, 0.34, foxTargetMask.r);
  float featherCandidate = smoothstep(0.04, 0.30, foxTargetMask.g);

  vec3 hatRelative = abs(
    (vFoxObjectPosition - vec3(-0.005, 0.695, 0.060))
  ...
  float hatHeight = smoothstep(0.555, 0.590, vFoxObjectPosition.y);
  float hatEarGuard = 1.0 - smoothstep(0.175, 0.215, vFoxObjectPosition.x);

  float featherX = smoothstep(0.030, 0.055, vFoxObjectPosition.x)
    * (1.0 - smoothstep(0.205, 0.225, vFoxObjectPosition.x));
  float featherY = smoothstep(0.625, 0.655, vFoxObjectPosition.y);
  float featherZ = smoothstep(0.025, 0.050, vFoxObjectPosition.z)
    * (1.0 - smoothstep(0.220, 0.250, vFoxObjectPosition.z));
  float featherSpace = featherX * featherY * featherZ;
  float featherTip = smoothstep(0.890, 0.910, vFoxObjectPosition.y);
  ...
  float hatShading = mix(0.48, 1.08, smoothstep(0.04, 0.92, baseLuma));
  vec3 colorizedHat = foxHatColor * hatShading;
  ...
  sampledDiffuseColor.rgb = mix(resultColor, zoneColor, foxHatDebugMode);
  diffuseColor *= sampledDiffuseColor;
#endif`);
  };
  material.customProgramCacheKey = () => "color-fox-hat-v10";
  material.needsUpdate = true;
}
export function ColorFoxLabViewer({ variant, showZones }: Props) {
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const hatMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      hatMask.flipY = false;
      hatMask.colorSpace = THREE.NoColorSpace;
      hatMask.wrapS = THREE.RepeatWrapping;
      hatMask.wrapT = THREE.RepeatWrapping;
      hatMask.generateMipmaps = false;
      hatMask.minFilter = THREE.LinearFilter;
      ...
      model.traverse((child) => {
        ...
        const clones = originals.map((item) => {
          ...
          colorizeHatFeather(clone, controls.color, hatMask, controls.debugMode);
    ...
    setup().catch((error) => {
      console.error("[ColorFoxLabViewer] color fox load failed", error);
  ...
  return (
    <div className="color-animal-viewer color-animal-viewer--single color-fox-viewer" role="group" aria-label={`${variant.name} 3D Fox`}>
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div className="color-animal-single__palette" aria-label="当前帽子颜色">
      ...
      {status !== "ready" ? (
        <div className={`color-animal-viewer__status color-animal-viewer__status--${status}`} role="status">
          {status === "loading" ? <><span className="color-animal-viewer__spinner" /><strong>正在加载 Color Fox</strong><span>{progress}%</span></> : <><strong>3D 加载失败</strong><button type="button" onClick={() => setRetryKey((value) => value + 1)}>重新尝试</button></>}
        </div>
      ) : null}
    </div>
  );
