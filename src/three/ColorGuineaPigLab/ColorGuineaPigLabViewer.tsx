...
} from "../material/createColorGuineaPigMaterials";
import { loadRoomEnvironment, loadToyModel, loadToyViewerRuntime } from "../ToyViewer/runtime";
import { assignColorGuineaPigBalloonZones } from "./semanticZones";

export type GuineaPigBalloonVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type Props = {
  variant: GuineaPigBalloonVariant;
  showZones: boolean;
};

type Status = "loading" | "ready" | "error";

type GuineaPigControls = {
  balloonColor: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-guinea-pig/model-mobile-v001.glb";
const MASK_URL = "/models/toys/color-guinea-pig/balloon-zones-mobile-v003.webp?v=3";

export function ColorGuineaPigLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<GuineaPigControls | null>(null);
  ...
  useEffect(() => {
    ...
    if (!controls) return;
    controls.balloonColor.set(variant.swatch).multiplyScalar(0.94);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const balloonMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        balloonMask.dispose();
        return;
      }
      prepareColorGuineaPigMaskTexture(THREE, balloonMask);
      ...
      camera.position.set(0, 0.03, 6.7);
      camera.lookAt(0, 0, 0);
      ...
      scene.add(new THREE.HemisphereLight(0xffffff, 0x7d6f67, 1.95));
      const key = new THREE.DirectionalLight(0xfffbf7, 1.7);
      key.position.set(-4.5, 6.2, 7);
      scene.add(key);
      const fill = new THREE.DirectionalLight(0xffe4d9, 0.36);
      ...
      const rotationParam = queryParams.get("rotation");
      ...
      const inspectionRotation = Number.isFinite(rotationValue)
        ...
        : null;
      const controls: GuineaPigControls = {
        balloonColor: new THREE.Color(variantRef.current.swatch).multiplyScalar(0.94),
      ...
      assignColorGuineaPigBalloonZones(THREE, model);
      const materials = cloneColorGuineaPigMaterials(
        THREE,
        model,
        controls.balloonColor,
        balloonMask,
      ...
      stage.rotation.y = inspectionRotation ?? -0.25;
      stage.add(model);
      scene.add(stage);

      const shadow = new THREE.Mesh(
        new THREE.CircleGeometry(1.25, 52),
      ...
      shadow.position.set(0, -1.67, 0.1);
      shadow.scale.y = 0.48;
      scene.add(shadow);
      ...
      let targetRotationY = inspectionRotation ?? -0.25;
      ...
      function handleWheel(event: WheelEvent) {
        ...
        targetZoom = THREE.MathUtils.clamp(targetZoom + event.deltaY * 0.004, 5.1, 8.4);
        idleUntil = performance.now() + 4200;
        needsRender = true;
      }
    ...
    setup().catch((error) => {
      console.error("[ColorGuineaPigLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-guinea-pig-viewer"
      role="group"
      aria-label={`${variant.name} 3D 豚鼠与气球`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      <div
        className="color-animal-single__palette color-guinea-pig-viewer__palette"
        aria-label="当前气球颜色"
      >
        <span style={{ background: variant.swatch }} />
        <strong>{showZones ? "三只气球联合遮罩" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Guinea Pig</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
