...
RUNTIME_DIR = ROOT / "public" / "models" / "toys" / "color-dino"
RUNTIME_MODEL = RUNTIME_DIR / "model-mobile-v001.glb"
OUTPUT = RUNTIME_DIR / "scarf-mask-mobile-v001.webp"
MASK_SIZE = 512
...
def red_strength(pixels: np.ndarray) -> np.ndarray:
    ...
    return np.minimum.reduce(
        (
            np.clip((red - 118.0) / 92.0, 0.0, 1.0),
            np.clip((red - green - 46.0) / 88.0, 0.0, 1.0),
            np.clip((red - blue - 42.0) / 94.0, 0.0, 1.0),
        )
    )
...
def main() -> None:
    ...
    print(
        ...
        f"({OUTPUT.stat().st_size} bytes, {coverage:.1%} scarf coverage)"
    )
