...
const MODEL_URL = "/models/toys/diamond-dog/model-mobile-v001.glb";

export function DiamondDogLabViewer({
  variant,
  inspectFacets,
  compact = false,
  interactive = true,
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, compactViewport ? 1.25 : 1.55));
      ...
      const controls: DiamondControls = {
        applyVariant: (swatch) => {
          applyDiamondUnicornTint(THREE, material, swatch, compact);
        },
        applyInspectionMode: (enabled) => {
          material.transmission = enabled || compact ? 0.42 : 0.95;
          material.roughness = enabled ? 0.16 : compact ? 0.14 : 0.025;
          material.dispersion = enabled || compact ? 0.01 : 0.12;
          material.thickness = enabled || compact ? 0.4 : 1.45;
      ...
      const scale = 3.25 / Math.max(size.y, size.x, size.z, 0.001);
      ...
      stage.rotation.set(-0.025, rotationController?.getRotation() ?? -0.24, 0);
      stage.add(centeredModel);
      scene.add(stage);

      const shadow = new THREE.Mesh(
        new THREE.CircleGeometry(1.32, 56),
      ...
      shadow.position.set(0, -1.72, 0.08);
      shadow.scale.y = 0.34;
      scene.add(shadow);
      ...
      function handleWheel(event: WheelEvent) {
        ...
        targetZoom = THREE.MathUtils.clamp(targetZoom + event.deltaY * 0.004, 4.9, 9.4);
        idleUntil = performance.now() + 5200;
        needsRender = true;
      }
    ...
    setup().catch((error) => {
      console.error("[DiamondDogLabViewer] diamond dog load failed", error);
  ...
  }, [compact, interactive, retryKey, rotationController]);

  return (
    <div
      className={`color-animal-viewer color-animal-viewer--single diamond-dog-viewer${compact ? " diamond-dog-viewer--compact" : ""}`}
      role="group"
      aria-label={`${variant.name} 3D 水晶小狗`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      {!compact ? (
        <div className="color-animal-single__palette" aria-label="当前水晶材质">
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Diamond Dog</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
