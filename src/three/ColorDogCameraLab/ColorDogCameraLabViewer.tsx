...
export type DogCameraAccessoryVariant = {
  id: string;
  name: string;
  swatch: string;
};
...
type Props = {
  variant: DogCameraAccessoryVariant;
  showZones: boolean;
  compact?: boolean;
  interactive?: boolean;
...
type DogCameraControls = {
  color: import("three").Color;
  debugMode: { value: number };
  invalidate: () => void;
};

const MODEL_URL = "/models/toys/color-dog-camera/model-mobile-v001.glb?v=1";
const MASK_URL = "/models/toys/color-dog-camera/accessory-mask-mobile-v001.webp?v=1";

export function ColorDogCameraLabViewer({
  variant,
  showZones,
  compact = false,
  interactive = true,
  ...
  const controlsRef = useRef<DogCameraControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      const accessoryMask = await new THREE.TextureLoader().loadAsync(MASK_URL);
      if (cancelled || !hostRef.current) {
        accessoryMask.dispose();
        return;
      }
      prepareColorDogCameraMaskTexture(THREE, accessoryMask);
      ...
      const controls: DogCameraControls = {
      ...
      const materials = cloneColorDogCameraMaterials(
        THREE,
        model,
        controls.color,
        accessoryMask,
    ...
    setup().catch((error) => {
      console.error("[ColorDogCameraLabViewer] model load failed", error);
  ...
  return (
    <div
      className={`color-animal-viewer color-animal-viewer--single color-dog-camera-viewer${compact ? " color-dog-camera-viewer--compact" : ""}`}
      role="group"
      aria-label={`${variant.name} 3D 狗狗相机`}
      data-status={status}
    >
      <div ref={hostRef} className="color-animal-viewer__host" />
      {!compact ? (
        <div className="color-animal-single__palette" aria-label="当前帽子和包包颜色">
          <span style={{ background: variant.swatch }} />
          <strong>{showZones ? "帽子与包包区域检查" : variant.name}</strong>
        </div>
      ) : null}
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Dog Camera</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
