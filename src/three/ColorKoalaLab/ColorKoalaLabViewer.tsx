...
const MODEL_URL = "/models/toys/color-koala/model-mobile-v001.glb?v=1";
const MASK_URL = "/models/toys/color-koala/hat-mask-mobile-v001.webp?v=3";

export function ColorKoalaLabViewer({ variant, showZones }: Props) {
  const hostRef = useRef<HTMLDivElement>(null);
  const controlsRef = useRef<KoalaControls | null>(null);
  ...
  useEffect(() => {
    ...
    async function setup() {
      ...
      prepareColorKoalaMaskTexture(THREE, hatMask);
      ...
      renderer.setPixelRatio(
        Math.min(window.devicePixelRatio, compactViewport ? 1.3 : 1.6)
      );
      ...
      const materials = cloneColorKoalaMaterials(
        THREE,
        model,
        controls.color,
        hatMask,
      ...
      stage.rotation.set(-0.02, -0.24, 0);
      ...
      function render(time: number) {
        ...
        if (document.hidden || time - lastFrame < 1000 / (dragging ? 60 : 30)) {
          return;
        }
    ...
    setup().catch((error) => {
      console.error("[ColorKoalaLabViewer] model load failed", error);
  ...
  return (
    <div
      className="color-animal-viewer color-animal-viewer--single color-koala-viewer"
      role="group"
      aria-label={`${variant.name} Color Koala 3D`}
      data-status={status}
    >
      ...
      <div className="color-animal-single__palette" aria-label="当前帽子颜色">
        ...
        <strong>{showZones ? "帽身区域检查" : variant.name}</strong>
      </div>
      {status === "ready" ? (
        ...
        >
          {status === "loading" ? (
            <>
              ...
              <strong>正在加载 Color Koala</strong>
              <span>{progress}%</span>
            </>
          ) : (
            <>
