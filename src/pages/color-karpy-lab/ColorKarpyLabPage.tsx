...
import { ColorKarpyLabViewer, type KarpyHatVariant } from "../../three/ColorKarpyLab/ColorKarpyLabViewer";

const variantPool: KarpyHatVariant[] = [
...
export function ColorKarpyLabPage() {
  ...
  return (
    <main className="color-animal-page color-animal-page--single color-karpy-page">
      ...
      <section className="color-animal-intro">
        <div>
          <p className="eyebrow">COLOR KARPY STUDY 01</p>
          <h1>Karpy 保持原色，只给头顶帽子换颜色。</h1>
          <p>Karpy 的身体、白色上衣、眼睛、鼻嘴和腮红始终保留原色。随机按钮只改变头顶红色帽子。</p>
        </div>
      ...
      <ColorKarpyLabViewer variant={variant} showZones={showZones} />
      <section className="color-animal-notes" aria-label="技术说明">
        <Info size={19} />
        <p><strong>帽子专用着色：</strong> 315,240 字节的 Draco 模型搭配 18,474 字节无损 WebP 帽子蒙版。只有红色帽子候选区与头顶三维范围同时命中时才会变色，Karpy 其余部分始终使用原始贴图。</p>
      </section>
    </main>
  );
}